...
State-of-the-art generative language models are now capable of producing multi-paragraph excerpts which, at first glance, readers find virtually indistinguishable from genuine human-written content \citep{zellers2019defending,radford2019language,adelani2020generating}.
Often, only subtle logical fallacies or idiosyncrasies of language reveal that text is machine-generated.
...
However, using detection as a method for evaluating NLG systems has several advantages over soliciting quality ratings.
...
If we instead use a decoding strategy that reduces the entropy of the distribution before sampling, the result is improved generation quality at the cost of diversity.
We show that humans have a hard type identifying that text is machine-generated when sampling is heavily restricted to only high-likelihood words, but automatic detection systems easily pick up on the resulting lexical anamolies.
In other words, humans are fooled where text classifiers are not, and vice versa.
...
I show how factors such as model size and text genre impact detectability, and I introduce ways to gamify the detection task.
...
Using top-$k$ random sampling, a decoding method where only the selection of high-likelihood words is permitted, means the system is less likely to make a poor choice and create the type of mistakes that are easy for humans to detect.
Since humans are not proficient at identifying when a model subtly favors certain phrases more often than a human author, they don't notice the over-representation of high-likelihood words in the generated text.
...
Top-$k$ in particular generates text that is easy for machines to detect but difficult for humans.
Thus, we observe the general trend: as the number of unlikely words available to be chosen is increased, humans become {\em better} at detecting fakes while automatic systems become {\em worse}.
...
When a decoding strategy severely modifies the unigram token distribution, as top-$k$ does, humans have trouble detecting the resultant generated text, but automatic classifiers find it the easiest to discriminate.
My research further suggests that classifiers are brittle; they generalize poorly when trained to discriminate samples from one strategy, but are then evaluated on samples from another.
...
Each dataset is approximately balanced between positive examples of machine-generated text and negative examples of human-written text.
...
While we use the GPT-2 \textsc{Large} model with 774M parameters, we found that similar trends hold in experiments with smaller language models.
At the time these experiments were conducted, GPT-2 was considered state-of-the art; it has subsequently been surpassed by other larger LMs.
Detection is more difficult on larger models.
...
By picking text that resembles GPT-2's training data, we ensure that our classifiers cannot simply take advantage of stylistic differences between the human-written text corpus and the kind of text GPT-2 was trained to generate.

For each decoding method, I constructed a training dataset by pairing 250,000 generated samples with 250,000 excerpts of web text.
5,000 additional paired samples are kept aside for validation and test datasets.
Excerpts with fewer than 192 WordPiece tokens \citep{wu2016google} were removed (excerpts might be quite short if the model produces an end-of-text token early on).
...
This study was designed to answer the former, simpler question. To this end, I created two datasets, one with no priming, and one with the minimum amount of prompting possible: a single token of web text.
This means that for every excerpt of web text in the training set, the dataset contained an excerpt of machine-generated text that starts with the same token.
Our experiments show that the ability of automatic detectors can be strongly impacted by even this short prompt.

To study the effect of generation length, I constructed variations of the above datasets by truncating all excerpts to ten possible lengths ranging from 2 to 192 WordPiece tokens \cite{wu2016google}. In total, we obtain sixty dataset variations: one per sampling method, truncation length, and choice of priming or no priming.
...
As a primary discriminator method, I employ a fine-tuned BERT classifier \citep{devlin2018bert}.
I fine-tuned one instance of BERT per dataset variation described above.
...
BERT-\textsc{Large} (cased) was fine-tuned on the task of labeling a sentence as human- or machine- generated.
...
% \textsc{BoWDisc} 
For each sequence, I computed a bag-of-words embedding where each dimension corresponds to a token in GPT-2's  50,000 token BPE vocabulary \citep{sennrich2016neural}, and contains a count of how many times that token appears in the text sequence. 
I then trained a logistic regression binary classifier to predict human- or machine-written given this 50,000-dimensional embedding.
I experimented with truncating embedding size by removing entries for infrequent vocabulary words, but this did not improve performance.

\paragraph{Histogram-of-Likelihood Ranks}
Following GLTR \citep{gehrmann2019gltr}, I computed the probability distribution of the next word given the previous words in a text sequence according to a trained language model (in our case the same GPT-2 model that was used for generation).
At each sequence position, I reranked the vocabulary words by likelihood, and record the rank of the ground-truth next word within this list.
...
However, I observed higher accuracy when 50 bins are spread uniformly over the possible rankings.
This means that since there were 50,000 vocabulary words, the first bin counted the number of times the actual next word was within the 1,000 mostly likely next words, the second bin counts the 1,001-2,000th, and so on.
I then trained logistic regression binary classifiers to predict human- or machine-written given either the 4-dimensional histograms or 50-dimensional histograms as input.
...
Raters were asked to decide whether a passage of text was written by a human or by a computer algorithm.
Figure \ref{fig:amt_screenshot} shows screenshots of the instructions and user interface for the annotation task.
Raters are allowed to choose between four options: (1) ``definitely machine-generated," (2) ``possibly machine-generated," (3)  ``definitely human-written," and (4) ``possibly" human-written.
...
The same study was then performed with university students who were first guided through ten examples (Table \ref{tab:expert_rater_training}) as a group.
...
As expected, BERT far surpasses all simple baselines, indicating that it is not fully possible to solve the detection problem without complex sequence-based understanding.
The simplest baseline, TotalProb, which makes a decision based on the likelihood of the sequence, performs surprisingly well relative to the more complex logistic regression models (achieving over 60\% accuracy for all sampling methods).
...
\paragraph{Fine-tuned BERT} In Figure~\ref{fig:bert_accuracy}, we see discriminator accuracy as a function of excerpt length and sampling method.
...
Top-$k$'s samples so easy to detect because there are only a small number of word sequences that can start a generation when we limit to only ever choosing the 40 most likely tokens at each generation step.
...
In fact, \citet{see2019massively} note that $k$ must be set to 1000 to achieve about the same amount of rare word usage and fraction of non-stopword text as as human writing.\footnote{when decoding from the GPT-2 small model with 117M parameters.}
This makes it very easy for the model to pick out machine-generated text based on these distributional differences.

Instead of unconditioned generation, which in actuality means always conditioning on the same thing (an empty sequence), we can instead prompt with human-written text that the NLG system then extends.
...
Without priming, a discriminator trained on sequences of length 2 can classify the provenance of the text with $\mathtt{\sim}$90\% accuracy (Figure \ref{fig:bert_accuracy}).
...
\begin{figure}[t]
    ...
    \caption{\textbf{(a)} and \textbf{(b)} show human rater accuracy of correctly identifying an excerpt as human-written or machine-written, shown with 80\% confidence internals, in \textbf{(a)}, broken up by decoding strategy and in \textbf{(b)}, overall. Accuracy increases as raters observe more tokens. \textbf{(c)} shows that for short excerpts, most rater mistakes occur when raters incorrectly think machine-generated text is human written. The two errors types become more balanced at longer lengths.}
...
This is seen in Figure \ref{fig:mean_ks} where at the very first step of unprimed generation, nucleus sampling selects from 3,075 possible vocabulary words, and at later positions selects from on average more than 500.
...
Unsurprisingly a discriminator trained on top-$k$ generalizes poorly to other sampling methods: accuracy drops to as low as 42.5\%, worse than chance.
...
Only the discriminator trained with nucleus sampling (a compromise between unmodified sampling and top-$k$) was able to detect sequences from the other sampling strategies without too much of a decline in accuracy.
...
\begin{table}[t]
  \small
  \centering
  \caption{Average probability of `machine-generated' according to each length-192 discriminator with one token of conditioning. The expected in-domain probability is 0.5.}
...
In Table~\ref{tab:transfer-prediction}, we observe that of the three BERT discriminators, only the one trained on top-$p$ samples predicts `machine-generated' on approximately 50\% of in-domain examples as expected. 
...
Even with the multi-paragraph 192-length excerpts, human performance is only at 71.4\%, indicating that even trained humans struggle to correctly identify machine-generated text over a quarter of the time.
...
To break up the accuracies by sampling method in a way that is comparable to the results shown for the automatic discriminators, we paired each machine-generated example with a randomly selected example of webtext to create a balanced dataset for each sampling strategy.
...
While improved detection models are inevitable, it is unclear how to improve human performance.
GLTR proposes providing visual aids to humans to improve their performance at detecting generated-text, but it is unlikely that their histogram-based color-coding will continue to be effective as generative methods become better at producing high-quality text that lacks statistical anomalies.
...
The pilot study of section described in Section \ref{section:detection} showed how choice of decoding strategy impacts human ability to detect machine-generated text.
However, there are many other factors which influence detectability that I was not able to include in this study, including the domain of the text being used for evaluation and the architecture and manner in which the underlying language model was trained.
In addition, it is important to also understand the annotators themselves--specifically, the impact educational and socioeconomic background has on detection skill, as well as the impact of incentive structure on detection performance.
We therefore saw the necessity of designing a platform for conducting large-scale studies of the detection task.

Previous studies, including my pilot study, focused on the binary classification task--given a text example that is either entirely human-written or entirely machine-generated (aside from an initial prompt), annotators must predict whether it is human-written or machine-generated.
...
For our large-scale study, we instead framed detection as a boundary-detection task: given a document that begins as human-written and at some point transitions to machine-generated, can annotators detect the transition point?
...
\begin{itemize}[itemsep=0.5pt,topsep=1pt]
    ...
    \item Do annotators who take longer per annotation or spend more time on the task perform better?
    \item Are there external factors (such as knowledge of NLG) which make some annotators better at the task?
\end{itemize}
...
\ROFT{} is a turn-based game in which a player first selects a domain of text (news articles, recipes, short stories, or speeches).
...
Once the sentences transition to being machine-generated, they will stay this way for the rest of the 10-sentence passage.
...
If the player deems the current sentence to be written by a machine, the game round ends and the true author (machine or human) for each sentence is revealed, potentially allowing the player to improve their intuitions over time.
...
This detection task is considerably more realistic than prior work, since in the real world, generating with a prompt is the standard way to achieve controllability, and malicious actors will not reveal what portion of a generation is the human-written prompt.
...
Finally, we discuss the difficulty in incentivizing players to improve over time.
...
We hypothesized that this domain would be challenging for players since the writing quality of the stories is not always high, and the presence of badly written stories lowers the bar for the model generation quality.
Also, factuality is not as important in a fictional domain.
...
We set the ``first sentence'' of each document as the name of the recipe and the ingredient list, and each subsequent ``sentence'' was a step in the recipe. Some recipe steps were more than one sentence. We hypothesized that this dataset would be difficult for models due to the amount of commonsense facts and causality present in recipes. 
...
Table \ref{tab:correlations_with_other_metrucs} shows the correlation between mean score and other metrics we could have chosen.
...
The second section (Group B) was explicitly told they would be awarded two points of extra credit toward their final course grades.
...
We also removed annotations from two players who cheated by exploiting Javascript vulnerabilities.
...
CTRL has the option to specify a control code indicating the domain in which to generate text in. For half of the generations, we used the ``[Politics]'' control code while for the other half we randomly selected a control code each time.
We expected use of the politics control code to result in generations that were more on topic.
...
For comparison, if a player guessed uniform randomly every round, their expected per-round score would be 1.31, and
...
Over all annotations, we found $\alpha$=-0.25, indicating there was less agreement than could be expected from random guessing.
This suggests that different annotators were better at identifying different kinds of problems with LM-generated text.
...
For Recipes, we expect that the task was made easier by the fact that the first human-written "sentence" in each game round was a semi-structured ingredients list, making it easy for players to check for contradictions---a step saying to \textit{mix in cream} is probably machine-generated if there is no cream ingredient.
...
We investigated the efficacy of these control codes on Speeches genre by using ``[Politics]'' for half the generations and randomly selecting control codes for the remaining half.
We found that use of the politics control code did not significantly affect  players' ability to distinguish real text from fake text.
This is not to say that control codes do not affect generation; however, it does suggest that the cues used by players to detect generations may not be related to genre-specific details, as least not within the Speeches genre.
...
While we can only speculate as to the amount of recipe knowledge present in the pre-trained model (GPT-2's training data is not publicly available), it is possible that the pre-trained model already contained enough understanding of recipe-like text, so that it was not critical to conduct extra finetuning.
We hypothesize that finetuning would have had more impact in a specialized or jargon-laden domain (e.g. legal, medical).
...
\begin{figure}[tb]
    ...
    \caption{Violin plots showing results of our mandatory exit survey. A violin plot is a box plot that also provides a density estimation. Results shown are filtered to only include players who completed at least 20 rounds. We see that reading the help guide, being a native English speaker, and providing a custom response for the question on familiarity with NLG all contribute to a higher mean score while high domain expertise does not seem have an affect (except in the case of Stories, where variance is lower for domain experts).}
...
\begin{figure}[tb]
    ...
    \caption{We see that the human sentences tend to have a different number of named entities than generated sentences.
...
To measure the duration of game rounds, we logged how many seconds players spent on each sentence decision.
...
While one might expect players to take more time to read and come to a decision on longer sentences, we found no correlation between time taken and length of sentence ($\rho$=-0.10).
This indicates that the limiting factor on player speed is not the time it takes to read the sentence.
...
We found surprisingly divergent trends across different genres.
On News and Recipes, the generated sentences tended to have fewer named entities than in human-written sentences.
Annotators did not pick up on these trends, though they may have noticed that for Stories, the generated sentences tended to have slightly more named entities.
...
Additionally in Stories, the generated sentences have on average a greater proportion of adjectives and adverbs, but annotators did not pick up on this trend.


\subsubsection{Does familiarity affect detectability?}
All participating players filled out an exit survey after completing their annotations. The questions on this survey are shown in Table \ref{tab:survey_questions}.
...
(For example, we released the extra credit assignment a week after a prominent NLG researcher gave a colloquium talk, and a couple of the responses referenced hearing about GPT in her talk.)
Second, participants who admitted that they did not read our help guide tended to perform poorly; all the best players did read the guide.
...
Finally, we did not observe any correlation between self-reported familiarity with a given genre and detection skill within that genre.
...
\begin{table}
    ...
    & Never (1) \\\midrule
    How often do you read news from credible news pub- & Daily (5) \\
    lishers (Wall Street Journal, New York Times, etc.)? & Once to a few times per week (4) \\
    & Once to a few times per month (3) \\
    & Once to a few times per year (2) \\
    & Never (1) \\\midrule
    How often do you read fiction on the internet (fan fiction & Daily (5) \\
    , creative writing sub-reddits, ebooks, etc.)? & Once to a few times per week (4) \\
    & Once to a few times per month (3) \\
    & Once to a few times per year (2) \\
    & Never (1) \\\midrule
...
\label{sec:reasons}
Each time a player designated a sentence as machine-generated, they had the option to specify why they made this decision, selecting from a set of pre-defined options (Table \ref{tab:reasons_text}) or else writing down a custom reason.
Table \ref{tab:reasons} shows the average number of points earned for each reason when that reason was specified.
...
In addition, we see that over 30\% of all reasons given for identifying generated text as generated are because the text is ``irrelevant or unrelated to the previous sentences.''
This result was consistent across all models and domains.
...
Figure \ref{tab:reasons_text} shows the full text of the explanations players could select for their boundary decisions, as well as some ``other'' responses we received. 
...
\begin{figure*}[tb]
    ...
    \caption{The reasons provided by players as to why a given example was generated broken up by generative model employed.}
...
While this trend was true in both domains in which we tested it, the impact of decoding strategy was more stark in the Stories domain than in the News domain.
In addition, we have provided new insights into the ways in which humans interact with partially-generated text.

Future work could expand our study by testing a larger set of models, genres, and other experimental conditions (finetuning, topic control, decoding strategy, etc.).
...
Future work may look at continuations that do not happen exactly on the boundary between sentences.
...
% For example, researchers should develop stratgies to guide people toward the correct language to properly explain how and why they come to select the sentences that they do, whether this be in the form of better error categorization or in free text responses. 
Finally, we acknowledge that university students (many of whom have studied computer science) may not be representative of the larger population.
It will be important for future work to broaden study participation to a more diverse population with the goal of understanding how the unique backgrounds of different annotators contribute to their ability to detect generated text.
...
% Many of the students who received extra credit proportional to the amount of points they scored learned they could exploit the point system by always picking one of the later sentences as the boundary.
...
% The player would have a certain number of tries to guess the index of the boundary sentence and would be scored based on the number of tries it takes to correctly complete the task.
...
However, the results in this section are limited in that they do not break down detection by text genre, do not investigate passages which are partially human-written and partially generated, and assume the automatic classifiers have access to training data from the same generative model we are aiming to detect samples from.

Section \ref{section:roft} aims to fill in several of these gaps by conducting a large-scale user study of human detection ability.
...
On one hand, gamifying and publicizing the detection task may help to raise the public's awareness of their susceptibility to machine-generated text, and work such as ours paves the way for future research on techniques for improving the public's detection abilities.
On the other hand, we show that the detection task is a viable method for evaluating NLG systems.
For researchers aiming to build better generative language models, decreasing human detection ability might be a reasonable goal to optimize for.
As much as my research seeks to better understand and improve human detection ability, it can can just as easily be used to make generative models even less detectable than they already are.
Despite this drawback, I nonetheless believe it is important to study detection as a means of assessing the risks that language models pose and protecting against future harm.
