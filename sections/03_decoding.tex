...
It has been a standard metric for chatbot evaluation \citep{lowe2017towards}, but has not been adopted more generally for evaluating natural language generation systems.
...
In Section \ref{section:detection}, we explore the relationship between choice of decoding strategy and the detectability of generated text.
When using a decoding strategy that samples from a language model's full predicted distribution, we end up frequently choosing words from the long tail of the distribution.
...
In Section \ref{section:roft}, we conduct a large-scale study of the detectability of generated text by human annoators, expanding upon the small-scale human evaluation experiments described in Section \ref{section:detection}.
We show how factors such as model size and text genrew impact detecability, and we propose ways to gamify the detection task.
