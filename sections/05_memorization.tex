...
\subsection{Definitions of Memorization}
\label{sec:mem-def}
...
% Extractive definitions of memorization are closely tied to the concept of plagiarism.
% % Ultimately, which has been under-studied in the language modeling literature, is whether humans perceive plagiarism to have occurred.
% When presented with two passages of text, humans are typically able to make judgement calls over whether the passages are similar enough to each other to be seen as plagiarism.
% Automatic methods for identifying memorization in generated sequences are really attempting to capture human intuition for identifying plagiarism.
...
\begin{figure}[h]
    \includegraphics[height=0.4\textwidth]{figures/exactly_mem-vs-model_size-prompt-all-gen-50-xlabel-ylabel.pdf} %[height=10.5em]
    \centering
...
To confirm that larger models are indeed \emph{memorizing} more data, and not simply \emph{generalizing} better, we also performed the same analysis with the GPT-2 model family as a baseline. 
%
...
\begin{figure}[ht]
        \includegraphics[height=0.4\textwidth]{figures/exactly_mem-vs-repetitions-mean-xlabel-markers.pdf} %[height=10.75em]
        \centering
...
\begin{figure}[h]
        \centering
        \includegraphics[height=0.4\textwidth]{figures/exactly_mem-vs-prompt_len-seq-500-gen-50-xlabel-markers.pdf} %[height=10.5em]
        \caption{
...
The discoverability phenomenon may seem natural: conditioning a model on $100$ tokens of context is more specific than conditioning the model on $50$ tokens of context, and it is expected that the model would estimate the probability of the training data as higher in this situation. 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/random_exactly_mem-vs-prompt_len-seq-varyseqlen-gen-50-xlabel-ylabel-markers} %[height=10.75em]
...
Much like before, we observed similar trends with model scale and context length.
...
The average difference in fraction of extractable memorization was just under 2 percentage points on average, with a maximum of 5.6. %, for the bucket containing sequences with about 1300 repetitions, and the average difference is around $2\%$.
Interestingly, beam search and greedy decoding generated the same output 45\% of the time.

The most common decoding strategy employed by modern LMs is \emph{random sampling}, in which the next token is selected at random according to a probability distribution derived from the model's predictions.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/beam_search_and_overall} %[height=10.75em]
    %\put(0,-7){(b)}
    \caption{
...
We refrained from using this approach for our main experiments
...
\subsection{Replication Study: T5}
...
\subsection{Replication Study: Language Models Trained on Deduplicated Data}
...
% Larger datasets tend to suffer in quality compared to their smaller predecessors because it is so expensive to perform manual review and curation on them.
...
Our experiments identify four distinct advantages to training on datasets that have been thoroughly deduplicated.
\begin{enumerate}
...
\end{itemize}
% Email: daphneipp@gmail.com
% Confirmation Code: b2m9hf
% Date Saved: 6/24/2022
...
For RealNews and LM1B, derived from news sites, we observed that many near-duplicates occur because the same news article appears on multiple news sites with slightly different formatting.
...
In Section \ref{section:dedup}, I presented a comprehensive quantitative analysis of the effect of mitigating the duplication problem.
